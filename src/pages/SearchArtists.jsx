...
import { Mic2, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function SearchArtists() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 pb-8">
      <h1 className="text-3xl font-bold text-white mb-1">Search Artists</h1>
      <p className="text-text-secondary text-sm mb-6">Discover artists by name</p>
      ...
      {!query && (
        <div className="flex flex-col items-center py-16">
          <Mic2 size={48} className="text-text-muted mb-4" />
          <p className="text-text-secondary">Type an artist name to start searching</p>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 md:gap-4">
            {artists.map((artist) => (
              <MediaCard
                ...
                item={artist}
                type="artist"
                subtitle={artist.role || artist.description}
              />
            ))}
          </div>
      ...
      {!loading && query && artists.length === 0 && !error && (
        <div className="flex flex-col items-center py-16">
          <Mic2 size={48} className="text-text-muted mb-4" />
          <p className="text-text-secondary">No artists found for &ldquo;{query}&rdquo;</p>
        </div>
      )}
    </div>
  );
