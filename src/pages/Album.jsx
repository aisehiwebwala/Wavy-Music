...
import { getImageUrl, getArtistNames, formatDuration, formatCount } from "../api/helpers";
...
export default function Album() {
  const { id } = useParams();
  const { data: album, loading, error, refetch } = useApi(() => getAlbum(id), [id]);
  ...
  const image = getImageUrl(album.image, "500x500");
    ...
    <div className="p-4 md:p-6 lg:p-8 pb-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        ...
        <div className="flex flex-col justify-end">
          <span className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Album</span>
          <h1 className="text-2xl md:text-5xl font-extrabold text-white mt-1 mb-2 break-words">{album.name}</h1>
          <div className="flex flex-wrap items-center gap-1 text-sm text-text-secondary">
            {album.artists?.primary?.[0] && (
              <Link to={`/artist/${album.artists.primary[0].id}`} className="font-semibold text-white hover:underline">
                {artistNames}
              </Link>
            )}
            {album.year && <span> &middot; {album.year}</span>}
            {album.songCount && <span> &middot; {album.songCount} songs</span>}
            {totalDuration > 0 && <span> &middot; {formatDuration(totalDuration)}</span>}
            {album.playCount && <span> &middot; {formatCount(album.playCount)} plays</span>}
          </div>
      ...
      <div className="flex items-center gap-3 px-4 py-2 text-xs text-text-muted uppercase tracking-wider border-b border-white/5 mb-1">
        ...
        <span className="hidden md:block flex-1 max-w-[200px]" />
